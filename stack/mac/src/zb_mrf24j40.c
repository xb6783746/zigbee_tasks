...
void zb_mrf_fifo_read(zb_uint8_t tx_fifo, zb_buf_t *buf, zb_uint8_t len)
{
	...
	if (len)	// read txfifo
	{
		...
		fifo_addr = (tx_fifo ? ZB_NORMAL_TX_FIFO : ZB_RX_FIFO);
		if (tx_fifo)
...
//	if (!tx_fifo)
	{
		...
		ZB_WRITE_SHORT_REG(ZB_SREG_RXFLUSH,reg | 0b00000001);
...
*/
	...
	ZB_CLEAR_TRANS_INT();
	reg = ZB_READ_SHORT_REG(ZB_SREG_ISRSTS);
	TRACE_MSG(TRACE_MAC1, "interrupt ISRSTS: 0x%hx" , (FMT__H, reg));
  ...
  /* check for a pending frame bit in received ACK, but only after DATA_REQUEST */
	...
	{
		reg = ZB_READ_SHORT_REG(ZB_SREG_TXNTRIG);
		TRACE_MSG(TRACE_COMMON2, "TXNTRIG read: 0x%hx", (FMT__H, reg));
		if (reg & 0x10)
		{
	...
	if (ZB_UBEC_GET_TX_DATA_STATUS())
	{
		...
		reg = ZB_READ_SHORT_REG(ZB_SREG_TXSR);
...
void send_any_sheet(uint8_t *p, int len)
{
	...
	ZB_WRITE_SHORT_REG(ZB_SREG_TXNTRIG,0x00);
	...
	ZB_MEMCPY(ptr,p,len);	// заливаем сам пакет
